...
abstract class ManifestTests : TestBase() {
    ...
    /**
     * Tests creating a valid c2pa.opened action assertion with a parent ingredient.
     *
     * Per C2PA spec, c2pa.opened indicates an existing asset was opened and is being set as the
     * parentOf ingredient. The action's `related` field references the ingredient label.
     */
    suspend fun testOpenedActionWithIngredient(): TestResult = withContext(Dispatchers.IO) {
        runTest("Opened Action with Ingredient") {
            try {
                val parentIngredient = Ingredient.parent(
                ...
                val manifest = ManifestDefinition(
                    title = "Edited Photo",
                    claimGeneratorInfo = listOf(
                        ClaimGeneratorInfo(name = "C2PA Android Test", version = "1.0.0"),
                    ),
                    assertions = listOf(
                        AssertionDefinition.actions(
                            listOf(
                                ActionAssertion(
                                    action = PredefinedAction.OPENED,
                                    related = listOf("Original Photo.jpg"),
                                ),
                                ActionAssertion(
                                    action = PredefinedAction.EDITED,
                                    softwareAgent = "PhotoEditor 3.0",
                ...
                // Verify ingredient round-trips
                if (parsed.ingredients.size != 1) {
                    return@runTest TestResult(
                        "Opened Action with Ingredient",
                        false,
                        "Ingredient count mismatch",
                        "Expected 1, got ${parsed.ingredients.size}",
                    )
                ...
                val openedAction = actionsAssertion.actions.find { it.action == PredefinedAction.OPENED.value }
                if (openedAction == null) {
                    return@runTest TestResult(
                        "Opened Action with Ingredient",
                        false,
                        "c2pa.opened action not found after round-trip",
                    )
                }

                if (openedAction.related != listOf("Original Photo.jpg")) {
                    return@runTest TestResult(
                        "Opened Action with Ingredient",
                        false,
                        "Related ingredient label not preserved",
                        "Expected ['Original Photo.jpg'], got: ${openedAction.related}",
                    )
                }

                // Sign with Builder to verify it produces a valid manifest
                val outputFile = File.createTempFile("opened-action-test", ".jpg")
                try {
                    Builder.fromJson(jsonString).use { builder ->
                        val sourceImageData = loadResourceAsBytes("pexels_asadphoto_457882")
                        ByteArrayStream(sourceImageData).use { sourceStream ->
                            FileStream(outputFile).use { destStream ->
                                ...
                                Signer.fromInfo(signerInfo).use { signer ->
                                    ...
                                    TestResult(
                                        "Opened Action with Ingredient",
                                        true,
                                        "c2pa.opened action with parent ingredient serializes, round-trips, and signs correctly",
                                        "File size: ${outputFile.length()} bytes",
                                    )
                                }
                            }
